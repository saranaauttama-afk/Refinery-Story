...
state = act(state, { type: 'build', cellIndex: LUBE_A, building: 'lubricantPlant' })
...
state = act(state, { type: 'set_pause', cellIndex: DISTILL, paused: true })
...
let lube = tick.lines.find((line) => line.cellIndex === LUBE_A)!
...
exact = act(exact, { type: 'build', cellIndex: LUBE_B, building: 'lubricantPlant' })
tick = runV3ProductionTick(fixture(exact, 120, 0), 25)
const [lineA, lineB] = [LUBE_A, LUBE_B].map((cell) => tick.lines.find((line) => line.cellIndex === cell)!)
...
exact = act(exact, { type: 'build', cellIndex: POWER, building: 'powerPlant' })
...
close(tick.lines.find((line) => line.cellIndex === LUBE_A)!.actualWork, 1)
close(tick.lines.find((line) => line.cellIndex === LUBE_B)!.actualWork, 1)
...
powered = act(powered, { type: 'set_pause', cellIndex: DISTILL, paused: false })
...
const distillLine = plan.lines.find((line) => line.cellIndex === DISTILL)!
...
let full = act(powered, { type: 'set_pause', cellIndex: LUBE_A, paused: true })
full = act(full, { type: 'set_pause', cellIndex: LUBE_B, paused: true })
full = act(full, { type: 'set_pause', cellIndex: DISTILL, paused: true })
...
let upgraded = act(powered, { type: 'upgrade', cellIndex: POWER })
...
assertBlocked(upgraded, { type: 'upgrade', cellIndex: POWER }, 'v3.upgrade.locked')

// ---- Storage: Lube Tank capacity and output-space bound ----
assert.equal(getV3ProductCapacity(upgraded, 'lubricants'), 200)
upgraded = act(upgraded, { type: 'build', cellIndex: 7, building: 'lubricantTank' })
assert.equal(getV3ProductCapacity(upgraded, 'lubricants'), 275)
upgraded = act(upgraded, { type: 'upgrade', cellIndex: 7 })
assert.equal(getV3ProductCapacity(upgraded, 'lubricants'), 390)
upgraded = act(upgraded, { type: 'set_pause', cellIndex: LUBE_A, paused: false })
...
const distillFeed = tick.lines.find((line) => line.cellIndex === DISTILL)!.feedstock
...
assertBlocked(tick.state, { type: 'demolish', cellIndex: 7, expectedBuilding: 'lubricantTank' }, 'v3.demolish.stock_overflow')

// ---- Workforce: one Operator improves exactly one line, demand follows work ----
let staffed = fixture(upgraded, 200, 200)
staffed = act(staffed, { type: 'assign_duty', employeeId: niranId, duty: { kind: 'line', cellIndex: LUBE_A } })
plan = evaluateV3Production(staffed, 25)
const staffedLine = plan.lines.find((line) => line.cellIndex === LUBE_A)!
...
close(plan.lines.find((line) => line.cellIndex === DISTILL)!.crewRate, 0, 'moved Operator leaves Distillation')
...
dev = cycles(act(dev, { type: 'set_pause', cellIndex: LUBE_A, paused: true }), 5)
...
assertBlocked(dev, { type: 'set_program', cellIndex: DISTILL, blueprintId: precisionLube!.id }, 'v3.program.invalid_blueprint')
dev = act(dev, { type: 'set_program', cellIndex: LUBE_A, blueprintId: precisionLube!.id })
...
let running = act(upgraded, { type: 'set_pause', cellIndex: DISTILL, paused: false })
running = act(running, { type: 'set_pause', cellIndex: LUBE_B, paused: false })
...
const beforeExpand = act(earnGasolineCash(a, a.world.moneyCents + 600_000).state, { type: 'assign_duty', employeeId: niranId, duty: { kind: 'line', cellIndex: DISTILL } })
...
assert.equal(expanded.world.grid[1], 'lubricantPlant')
assert.equal(expanded.world.grid[5], 'distillationUnit', '(1,1) moves from 4 to 5')
assert.equal(expanded.plantPrograms[5].cellIndex, 5)
assert.deepEqual(expanded.employeeDuties[niranId], { kind: 'line', cellIndex: 5 }, 'duty follows its line')
