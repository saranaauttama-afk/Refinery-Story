...
state = assertBlocked(state, { type: 'unlock_land_parcel', parcelId: 'ring1:north' }, 'v3.land.insufficient_cash')
...
// ---- Land parcel + move: new land, then relocate a line without losing state ----
const beforeLand = ((__s) => act(__s, { type: 'assign_duty', employeeId: niranId, duty: { kind: 'line', buildingId: slotId(__s, LUBE_A) } }))(earnGasolineCash(a, a.world.moneyCents + 150_000).state)
const lubeId = slotId(beforeLand, LUBE_A)
const planBefore = evaluateV3Production(beforeLand, 25)
assertBlocked(beforeLand, { type: 'move_building', buildingId: lubeId, x: 56, y: 45 }, 'v3.place.locked_land')
const landed = act(beforeLand, { type: 'unlock_land_parcel', parcelId: 'ring1:east' })
assert.equal(beforeLand.world.moneyCents - landed.world.moneyCents, 150_000)
assert.equal(landed.operatingLedger.capexCents - beforeLand.operatingLedger.capexCents, 150_000)
const moved = act(landed, { type: 'move_building', buildingId: lubeId, x: 55, y: 45 })
assert.deepEqual([moved.world.buildingsById[lubeId].x, moved.world.buildingsById[lubeId].y], [55, 45])
assert.equal(moved.world.buildingsById[lubeId].level, beforeLand.world.buildingsById[lubeId].level)
assert.deepEqual(moved.plantPrograms[lubeId], beforeLand.plantPrograms[lubeId], 'program follows the building')
assert.deepEqual(moved.employeeDuties[niranId], { kind: 'line', buildingId: lubeId }, 'duty follows the building')
assert.deepEqual(moved.variantInventory, landed.variantInventory, 'moving never touches stock')
assert.equal(moved.world.moneyCents, landed.world.moneyCents, 'no move exploit or cost')
assert.deepEqual(
  evaluateV3Production(moved, 25).lines.map((line) => [line.buildingId, line.actualWork]),
  planBefore.lines.map((line) => [line.buildingId, line.actualWork]),
  'moving does not change production',
)
assertBlocked(moved, { type: 'unlock_land_parcel', parcelId: 'ring2:east' }, 'v3.land.locked')
assert.equal(parseV3GameState(JSON.parse(JSON.stringify(moved))).status, 'loaded')
void attempt

console.log('PASS: V3-11a Lube recipe, site/generator power, reservation, tanks, crew, Fleet trial, development, reload, land parcel + move')
