...
state = assertBlocked(state, { type: 'build', buildingId: LUBE_A, building: 'jetFuelPlant' }, 'v3.build.locked')
...
state = act(state, { type: 'build', buildingId: LUBE_A, building: 'lubricantPlant' })
assert.equal(state.plantPrograms[LUBE_A].blueprintId, LUBE)
assert.equal(state.plantPrograms[LUBE_A].installedModule, 'none')
...
state = act(state, { type: 'set_pause', buildingId: DISTILL, paused: true })
...
let lube = tick.lines.find((line) => line.buildingId === LUBE_A)!
...
exact = act(exact, { type: 'build', buildingId: LUBE_B, building: 'lubricantPlant' })
tick = runV3ProductionTick(fixture(exact, 120, 0), 25)
const [lineA, lineB] = [LUBE_A, LUBE_B].map((cell) => tick.lines.find((line) => line.buildingId === cell)!)
...
exact = act(exact, { type: 'build', buildingId: POWER, building: 'powerPlant' })
...
close(tick.lines.find((line) => line.buildingId === LUBE_A)!.actualWork, 1)
close(tick.lines.find((line) => line.buildingId === LUBE_B)!.actualWork, 1)
...
powered = act(powered, { type: 'set_pause', buildingId: DISTILL, paused: false })
...
const distillLine = plan.lines.find((line) => line.buildingId === DISTILL)!
...
let full = act(powered, { type: 'set_pause', buildingId: LUBE_A, paused: true })
full = act(full, { type: 'set_pause', buildingId: LUBE_B, paused: true })
full = act(full, { type: 'set_pause', buildingId: DISTILL, paused: true })
...
let upgraded = act(powered, { type: 'upgrade', buildingId: POWER })
...
assertBlocked(upgraded, { type: 'upgrade', buildingId: POWER }, 'v3.upgrade.locked')

// ---- Storage: Lube Tank capacity and output-space bound ----
assert.equal(getV3ProductCapacity(upgraded, 'lubricants'), 200)
upgraded = act(upgraded, { type: 'build', buildingId: 7, building: 'lubricantTank' })
assert.equal(getV3ProductCapacity(upgraded, 'lubricants'), 275)
upgraded = act(upgraded, { type: 'upgrade', buildingId: 7 })
assert.equal(getV3ProductCapacity(upgraded, 'lubricants'), 390)
upgraded = act(upgraded, { type: 'set_pause', buildingId: LUBE_A, paused: false })
...
const distillFeed = tick.lines.find((line) => line.buildingId === DISTILL)!.feedstock
...
assertBlocked(tick.state, { type: 'demolish', buildingId: 7, expectedBuilding: 'lubricantTank' }, 'v3.demolish.stock_overflow')

// ---- Workforce: one Operator improves exactly one line, demand follows work ----
let staffed = fixture(upgraded, 200, 200)
staffed = act(staffed, { type: 'assign_duty', employeeId: niranId, duty: { kind: 'line', buildingId: LUBE_A } })
plan = evaluateV3Production(staffed, 25)
const staffedLine = plan.lines.find((line) => line.buildingId === LUBE_A)!
...
close(plan.lines.find((line) => line.buildingId === DISTILL)!.crewRate, 0, 'moved Operator leaves Distillation')
...
dev = cycles(act(dev, { type: 'set_pause', buildingId: LUBE_A, paused: true }), 5)
...
assertBlocked(dev, { type: 'set_program', buildingId: DISTILL, blueprintId: precisionLube!.id }, 'v3.program.invalid_blueprint')
dev = act(dev, { type: 'set_program', buildingId: LUBE_A, blueprintId: precisionLube!.id })
assert.equal(dev.plantPrograms[LUBE_A].setupRemainingTicks, 25)

// ---- Reload/recovery determinism and conservation over a running factory ----
let running = act(upgraded, { type: 'set_pause', buildingId: DISTILL, paused: false })
running = act(running, { type: 'set_pause', buildingId: LUBE_B, paused: false })
...
const beforeExpand = act(earnGasolineCash(a, a.world.moneyCents + 600_000).state, { type: 'assign_duty', employeeId: niranId, duty: { kind: 'line', buildingId: DISTILL } })
...
assert.equal(getV3BuildingType(expanded, 1), 'lubricantPlant')
assert.equal(getV3BuildingType(expanded, 5), 'distillationUnit', '(1,1) moves from 4 to 5')
assert.equal(expanded.plantPrograms[5].buildingId, 5)
assert.deepEqual(expanded.employeeDuties[niranId], { kind: 'line', buildingId: 5 }, 'duty follows its line')
