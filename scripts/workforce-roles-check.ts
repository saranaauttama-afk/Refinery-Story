...
for (const role of ROLES) {
  ...
  for (const plant of PLANTS) {
    const result = attempt(state, { type: 'assign_duty', employeeId: id, duty: { kind: 'line', buildingId: slotId(state, plantCells[plant]) } })
    ...
    if (allowed) {
      const rate = getV3LocalCrewRate(result.state, slotId(result.state, plantCells[plant]))
...
const before = evaluateV3Production(spec, 25).lines.find((line) => line.buildingId === slotId(spec, 0))!
spec = act(spec, { type: 'choose_specialization', path: 'green' })
const after = evaluateV3Production(spec, 25).lines.find((line) => line.buildingId === slotId(spec, 0))!
...
const lubeLine = specTick.lines.find((line) => line.buildingId === slotId(spec, 0))!
...
// ---- Fresh-save only: older preview revisions are rejected, never migrated ----
const rev7 = JSON.parse(JSON.stringify(createInitialV3GameState()))
rev7.schemaRevision = 7
assert.notEqual(parseV3GameState(rev7).status, 'loaded')
