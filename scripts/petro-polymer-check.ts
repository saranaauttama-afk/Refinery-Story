...
state = assertBlocked(state, { type: 'build', cellIndex: 0, building: 'polymerPlant' }, 'v3.build.requires_route')
...
close(tick.lines.find((line) => line.cellIndex === 0)!.actualWork, 0.8, 'site power 4 < Petro 5/cycle')
assert.equal(tick.lines.find((line) => line.cellIndex === 0)!.limitedBy, 'power')
state = act(state, { type: 'build', cellIndex: 1, building: 'powerPlant' })
tick = runV3ProductionTick(resources(state, { feedstock: 100, electricity: 80 }), 25)
const petroLine = tick.lines.find((line) => line.cellIndex === 0)!
...
state = act(state, { type: 'build', cellIndex: 2, building: 'polymerPlant' })
...
const polymer = tick.lines.find((line) => line.cellIndex === 2)!
...
close(tick.lines.find((line) => line.cellIndex === 2)!.actualWork, 0)
assert.equal(tick.lines.find((line) => line.cellIndex === 2)!.limitedBy, 'input')
...
let shared = act(poly, { type: 'build', cellIndex: 3 + 3, building: 'lubricantPlant' })
...
const lubeWork = plan.find((line) => line.cellIndex === 6)!.actualWork
const polyWork = plan.find((line) => line.cellIndex === 2)!.actualWork
...
let waste = act(state, { type: 'build', cellIndex: 7, building: 'wasteTreatmentPlant' })
waste = resources(waste, { waste: 40, electricity: 80 })
tick = runV3ProductionTick(waste, 25)
const treat = tick.lines.find((line) => line.cellIndex === 7)!
...
assertBlocked(tick.state, { type: 'set_module', cellIndex: 7, module: 'economy' }, 'v3.module.invalid_cell')
...
waste = act(waste, { type: 'build', cellIndex: 8, building: 'recyclingBunker' })
...
let staffing = act(state, { type: 'build', cellIndex: 7, building: 'wasteTreatmentPlant' })
...
for (const role of roles) {
  for (const [building, cellIndex] of cells) {
    const result = attempt(staffing, { type: 'assign_duty', employeeId: person(role).id, duty: { kind: 'line', cellIndex } })
...
let dev = act(c4(), { type: 'build', cellIndex: 0, building: 'laboratory' })
dev = act(dev, { type: 'upgrade', cellIndex: 0 })
dev = act(dev, { type: 'upgrade', cellIndex: 0 })
...
dev = act(dev, { type: 'start_development', family: 'petrochemicals', profile: 'precision', module: 'precision', knowledgeRank: 2, leadEmployeeId: null, labCellIndex: 0 })
...
dev = act(dev, { type: 'start_development', family: 'plasticPellets', profile: 'volume', module: 'none', knowledgeRank: 0, leadEmployeeId: null, labCellIndex: 0 })
...
grid = act(grid, { type: 'build', cellIndex: 15, building: 'petrochemicalPlant' })
...
let chain = act(shared, { type: 'set_pause', cellIndex: 0, paused: false })
chain = act(chain, { type: 'set_pause', cellIndex: 4, paused: false })
chain = act(chain, { type: 'build', cellIndex: 7, building: 'wasteTreatmentPlant' })
