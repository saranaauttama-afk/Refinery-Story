...
import { act, assertBlocked, slotId, attempt, close, cycles, materialBasisTotal, SLOT } from './v3-check-helpers'
...
let shared = act(poly, { type: 'build', buildingId: slotId(poly, 3) + 3, building: 'lubricantPlant' })
...
grid = act(grid, { type: 'build', ...SLOT(15), building: 'petrochemicalPlant' })
...
assert.equal(getV3BuildingType(grid, slotId(grid, 18)), 'petrochemicalPlant', '(3,3) keeps its row/column')
assert.equal(grid.plantPrograms[slotId(grid, 18)].buildingId, 18)
