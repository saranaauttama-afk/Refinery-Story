...
import { act, assertBlocked, attempt, close, cycles, materialBasisTotal } from './v3-check-helpers'
...
state = assertBlocked(state, { type: 'build', buildingId: 0, building: 'polymerPlant' }, 'v3.build.requires_route')
state = act(state, { type: 'build', buildingId: 0, building: 'petrochemicalPlant' })
assert.equal(state.plantPrograms[0].blueprintId, PETRO)
let tick = runV3ProductionTick(resources(state, { feedstock: 100, electricity: 0 }), 25)
close(tick.lines.find((line) => line.buildingId === 0)!.actualWork, 0.8, 'site power 4 < Petro 5/cycle')
assert.equal(tick.lines.find((line) => line.buildingId === 0)!.limitedBy, 'power')
state = act(state, { type: 'build', buildingId: 1, building: 'powerPlant' })
tick = runV3ProductionTick(resources(state, { feedstock: 100, electricity: 80 }), 25)
const petroLine = tick.lines.find((line) => line.buildingId === 0)!
...
state = act(state, { type: 'build', buildingId: 2, building: 'polymerPlant' })
...
const polymer = tick.lines.find((line) => line.buildingId === 2)!
...
close(tick.lines.find((line) => line.buildingId === 2)!.actualWork, 0)
assert.equal(tick.lines.find((line) => line.buildingId === 2)!.limitedBy, 'input')
...
let shared = act(poly, { type: 'build', buildingId: 3 + 3, building: 'lubricantPlant' })
...
const lubeWork = plan.find((line) => line.buildingId === 6)!.actualWork
const polyWork = plan.find((line) => line.buildingId === 2)!.actualWork
...
let waste = act(state, { type: 'build', buildingId: 7, building: 'wasteTreatmentPlant' })
waste = resources(waste, { waste: 40, electricity: 80 })
tick = runV3ProductionTick(waste, 25)
const treat = tick.lines.find((line) => line.buildingId === 7)!
...
assertBlocked(tick.state, { type: 'set_module', buildingId: 7, module: 'economy' }, 'v3.module.invalid_cell')
...
waste = act(waste, { type: 'build', buildingId: 8, building: 'recyclingBunker' })
...
let staffing = act(state, { type: 'build', buildingId: 7, building: 'wasteTreatmentPlant' })
...
for (const role of roles) {
  for (const [building, buildingId] of cells) {
    const result = attempt(staffing, { type: 'assign_duty', employeeId: person(role).id, duty: { kind: 'line', buildingId } })
...
let dev = act(c4(), { type: 'build', buildingId: 0, building: 'laboratory' })
dev = act(dev, { type: 'upgrade', buildingId: 0 })
dev = act(dev, { type: 'upgrade', buildingId: 0 })
dev = act(dev, { type: 'buy_research', researchId: 'premiumFuel' })
dev = act(dev, { type: 'buy_research', researchId: 'advancedProcessing' })
assert.equal(getV3AvailableKnowledgeRank(dev, 0), 2)
dev = addV3VariantInventory(dev, PETRO, 10, 1_000).state
dev = act(dev, { type: 'start_development', family: 'petrochemicals', profile: 'precision', module: 'precision', knowledgeRank: 2, leadEmployeeId: null, labBuildingId: 0 })
...
dev = act(dev, { type: 'start_development', family: 'plasticPellets', profile: 'volume', module: 'none', knowledgeRank: 0, leadEmployeeId: null, labBuildingId: 0 })
assert.equal(dev.developmentProject?.feeDebitedCents, 40_000)
assert.equal(dev.developmentProject?.remainingTicks, 200)

// ---- 5×5 expansion: C4, $25,000, 25-lot spatial budget ----
let grid = act(c4(), { type: 'expand_grid' })
grid = act(grid, { type: 'build', buildingId: 15, building: 'petrochemicalPlant' })
const before5 = grid.world.moneyCents
grid = act(grid, { type: 'expand_grid' })
assert.equal(grid.world.grid.length, 25)
assert.equal(before5 - grid.world.moneyCents, 2_500_000)
assert.equal(getV3BuildingType(grid, 18), 'petrochemicalPlant', '(3,3) keeps its row/column')
assert.equal(grid.plantPrograms[18].buildingId, 18)
...
let chain = act(shared, { type: 'set_pause', buildingId: 0, paused: false })
chain = act(chain, { type: 'set_pause', buildingId: 4, paused: false })
chain = act(chain, { type: 'build', buildingId: 7, building: 'wasteTreatmentPlant' })
...
bogus.plantPrograms[0].blueprintId = 'commodity:recycledMaterial'
...
console.log('PASS: V3-14a/b Petro, Polymer protection/basis, shared power, Waste Treatment, asphalt, roles, rank2 Q75, 5x5, conservation, single inventory writer')
