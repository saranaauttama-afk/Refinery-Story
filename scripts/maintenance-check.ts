...
import { act, assertBlocked, slotId, close, SLOT } from './v3-check-helpers'

const perCycle = (dollars: number) => dollars * 100 * 0.002 / 12
const line = (state: V3GameState, slot: number) => evaluateV3Maintenance(state).lines.find((entry) => entry.buildingId === slotId(state, slot))!

/** Labelled fixture: chapter/cash set directly to isolate the maintenance rule. */
function fixtureAt(chapter: 0 | 1 | 2 | 3 | 4, moneyCents = 10_000_000): V3GameState {
...
let c1 = act(fixtureAt(1), { type: 'build', ...SLOT(0), building: 'laboratory' })
...
c2 = act(c2, { type: 'build', ...SLOT(0), building: 'lubricantPlant' })
c2 = act(c2, { type: 'build', ...SLOT(1), building: 'lubricantTank' })
close(line(c2, 0).dueCents, perCycle(5_000))
c2 = act(c2, { type: 'upgrade', buildingId: slotId(c2, 0) })
close(line(c2, 0).dueCents, perCycle(5_000) * 1.4, 'processing Lv2 ×1.4')
c2 = act(c2, { type: 'upgrade', buildingId: slotId(c2, 1) })
close(line(c2, 1).dueCents, perCycle(750) * 1.25, 'storage Lv2 ×1.25')
const paused = act(c2, { type: 'set_pause', buildingId: slotId(c2, 0), paused: true })
...
let cuts = act(c2, { type: 'build', ...SLOT(2), building: 'maintenanceWorkshop' })
// V3-15.5: one Workshop per yard (building limit), so upkeep cuts cannot stack.
cuts = assertBlocked(cuts, { type: 'build', ...SLOT(6), building: 'maintenanceWorkshop' }, 'v3.place.building_limit')
close(evaluateV3Maintenance(cuts).globalCut, 0.05)
cuts = act(cuts, { type: 'upgrade', buildingId: slotId(cuts, 2) })
...
local = act(local, { type: 'assign_duty', employeeId: skilled.id, duty: { kind: 'line', buildingId: slotId(local, 0) } })
...
let broke = act(c2, { type: 'upgrade', buildingId: slotId(c2, 4) })
broke = act(broke, { type: 'set_program', buildingId: slotId(broke, 4), blueprintId: V3_DEFAULT_BLUEPRINT_ID.gasoline })
...
assert.deepEqual(broke.maintenanceEmergency, { sinceTick: broke.world.tickCount - 25, buildingId: V3_STARTER_BUILDINGS.distillationUnit.id })
assert.ok(broke.world.moneyCents >= 0, 'no negative money / debt')
const emergencyPlan = evaluateV3Production(broke, 25).lines
const emergencyLine = emergencyPlan.find((entry) => entry.buildingId === V3_STARTER_BUILDINGS.distillationUnit.id)!
...
assert.equal(emergencyPlan.find((entry) => entry.buildingId === slotId(broke, 0))!.status, 'paused', 'other lines paused')
assert.equal(getV3BuildingLevel(broke, slotId(broke, 4)), 2, 'real level retained')
...
assert.equal(evaluateV3Production(restored, 25).lines.find((entry) => entry.buildingId === slotId(restored, 0))!.status, 'ready')
assertBlocked(restored, { type: 'restore_operations' }, 'v3.maintenance.not_in_emergency')

// ---- Fresh-save only: old revisions are rejected ----
const rev9 = JSON.parse(JSON.stringify(createInitialV3GameState()))
rev9.schemaRevision = 9
assert.notEqual(parseV3GameState(rev9).status, 'loaded')
