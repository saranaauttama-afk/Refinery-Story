...
cuts = act(cuts, { type: 'build', ...SLOT(6), building: 'maintenanceWorkshop' })
...
assert.equal(emergencyPlan.find((entry) => entry.buildingId === 0)!.status, 'paused', 'other lines paused')
...
assert.equal(evaluateV3Production(restored, 25).lines.find((entry) => entry.buildingId === 0)!.status, 'ready')
