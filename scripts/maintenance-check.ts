...
const line = (state: V3GameState, buildingId: number) => evaluateV3Maintenance(state).lines.find((entry) => entry.buildingId === buildingId)!
...
let c1 = act(at(1), { type: 'build', buildingId: 0, building: 'laboratory' })
...
c2 = act(c2, { type: 'build', buildingId: 0, building: 'lubricantPlant' })
c2 = act(c2, { type: 'build', buildingId: 1, building: 'lubricantTank' })
close(line(c2, 0).dueCents, perCycle(5_000))
c2 = act(c2, { type: 'upgrade', buildingId: 0 })
close(line(c2, 0).dueCents, perCycle(5_000) * 1.4, 'processing Lv2 ×1.4')
c2 = act(c2, { type: 'upgrade', buildingId: 1 })
close(line(c2, 1).dueCents, perCycle(750) * 1.25, 'storage Lv2 ×1.25')
const paused = act(c2, { type: 'set_pause', buildingId: 0, paused: true })
...
let cuts = act(c2, { type: 'build', buildingId: 2, building: 'maintenanceWorkshop' })
cuts = act(cuts, { type: 'build', buildingId: 6, building: 'maintenanceWorkshop' })
close(evaluateV3Maintenance(cuts).globalCut, 0.05, 'two Lv1 workshops do not stack')
cuts = act(cuts, { type: 'upgrade', buildingId: 2 })
...
local = act(local, { type: 'assign_duty', employeeId: skilled.id, duty: { kind: 'line', buildingId: 0 } })
...
let broke = act(c2, { type: 'upgrade', buildingId: 4 })
broke = act(broke, { type: 'set_program', buildingId: 4, blueprintId: V3_DEFAULT_BLUEPRINT_ID.gasoline })
...
assert.deepEqual(broke.maintenanceEmergency, { sinceTick: broke.world.tickCount - 25, buildingId: 4 })
assert.ok(broke.world.moneyCents >= 0, 'no negative money / debt')
const emergencyPlan = evaluateV3Production(broke, 25).lines
const emergencyLine = emergencyPlan.find((entry) => entry.buildingId === 4)!
...
assert.equal(getV3BuildingLevel(broke, 4), 2, 'real level retained')
