...
assert.deepEqual(getV3IndustryScore(fresh), { total: 0, fame: 0, business: 0, recipes: 0 })
assert.equal(getV3PlayerRank(fresh), 5)
const strong: V3GameState = { ...fresh, world: { ...fresh.world, reputation: 100 } }
assert.equal(getV3PlayerRank(strong), 1, '1,000 fame points lead at the start')
...
assert.equal(year.world.reputation, 100 + 3 + 5 + 10, 'grade "-" gives 0; ranks 3+5+10')
