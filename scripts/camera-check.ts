...
// Plant/grid contract: a sprite is derived only from its tile contact point,
// so showing the Build grid cannot change its rectangle. The three starter
// families keep one gameplay footprint and contact point, while visual mass
// grows slightly per level so denser upgrades never look squeezed.
const tile = { x: 240, y: 360, width: 126, height: 63 }
for (const building of ['distillationUnit', 'crudeTank', 'gasolineTank'] as const) {
  const levelRects = [1, 2, 3].map((level) => {
    const profile = getPlantSpriteProfile(building, level)
    ...
    assert.deepEqual(duringBuild, beforeBuild, `${building} L${level} must not move in Build mode`)
  ...
  assert.ok(levelRects[0].size < levelRects[1].size)
  assert.ok(levelRects[1].size < levelRects[2].size)
  assert.ok(levelRects[2].size <= tile.width * 1.18, `${building} L3 should not hide its service road`)
}
