...
export const text = {
  ...
  settingsScreen: {
    title: bilingual('Settings', 'ตั้งค่า'),
    language: bilingual('Language', 'ภาษา'),
    languageNote: bilingual(
      'Applies across the whole game.',
      'มีผลกับทั้งเกม',
    ),
    audio: bilingual('Audio', 'เสียง'),
    soundLabel: bilingual('Sound effects', 'เสียงเอฟเฟกต์'),
    soundDesc: bilingual('Button taps, sales, and notifications', 'เสียงกดปุ่ม การขาย และการแจ้งเตือน'),
    musicLabel: bilingual('Music', 'เพลง'),
    musicDesc: bilingual('Background music while playing', 'เพลงประกอบระหว่างเล่น'),
    audioNote: bilingual(
      'Demo toggles — this build has no audio assets yet, so "Sound effects" doesn\'t play anything audible, but it does control haptic feedback. "Music" doesn\'t change anything yet.',
      'สวิตช์เดโม — บิลด์นี้ยังไม่มีไฟล์เสียง "เสียงเอฟเฟกต์" จึงยังไม่มีเสียงจริง แต่ควบคุมการสั่น (haptic) ได้ ส่วน "เพลง" ยังไม่มีผล',
    ),
    store: bilingual('Store', 'ร้านค้า'),
    adsRemoved: bilingual('✓ Ads removed', '✓ ลบโฆษณาแล้ว'),
    removeAds: bilingual('Remove Ads / Purchases', 'ลบโฆษณา / ซื้อ'),
    saveData: bilingual('Save data', 'ข้อมูลเซฟ'),
    resetSave: bilingual('Reset save', 'รีเซ็ตเซฟ'),
    resetConfirmTitle: bilingual('Reset save?', 'รีเซ็ตเซฟ?'),
    resetConfirmBody: bilingual('This deletes all progress and starts a new refinery.', 'จะลบความคืบหน้าทั้งหมดและเริ่มโรงกลั่นใหม่'),
    about: bilingual('About', 'เกี่ยวกับ'),
    aboutTagline: bilingual('A Kairosoft-style refinery management game.', 'เกมบริหารโรงกลั่นสไตล์ Kairosoft'),
  },
  storeScreen: {
    title: bilingual('Store', 'ร้านค้า'),
    removeAdsSection: bilingual('Remove Ads', 'ลบโฆษณา'),
    rewardedSection: bilingual('Rewarded Ads', 'โฆษณาแลกรางวัล'),
    boostsSection: bilingual('Boosts', 'บูสต์'),
    owned: bilingual('Owned', 'เป็นเจ้าของแล้ว'),
    soon: bilingual('Soon', 'เร็ว ๆ นี้'),
    rewardedNote: bilingual('Coming soon — watch a short video ad to earn these rewards for free.', 'เร็ว ๆ นี้ — ดูวิดีโอสั้น ๆ เพื่อรับรางวัลเหล่านี้ฟรี'),
    restore: bilingual('Restore Purchases', 'กู้คืนการซื้อ'),
    restoreTitle: bilingual('Restore purchases', 'กู้คืนการซื้อ'),
    restoreBody: bilingual('No previous purchases found on this device.', 'ไม่พบการซื้อก่อนหน้าบนอุปกรณ์นี้'),
    demoNote: bilingual(
      'Demo store — no real payment processor is connected. "Remove Ads" persists locally; the cash/RP boosts above are for illustration only and don\'t currently grant rewards.',
      'ร้านค้าเดโม — ยังไม่ได้เชื่อมระบบชำระเงินจริง "ลบโฆษณา" บันทึกในเครื่อง ส่วนบูสต์เงิน/RP ข้างบนเป็นตัวอย่างเท่านั้น ยังไม่ให้รางวัลจริง',
    ),
    buyTitle: (name: string) => bilingual(`Buy ${name}?`, `ซื้อ ${name}?`),
    buyBody: (desc: string, price: string) =>
      bilingual(
        `${desc}\n\nPrice: ${price}\n\n(This is a demo — no real payment will be made.)`,
        `${desc}\n\nราคา: ${price}\n\n(นี่คือเดโม — จะไม่มีการชำระเงินจริง)`,
      ),
    buy: bilingual('Buy', 'ซื้อ'),
    purchaseComplete: bilingual('Purchase complete', 'ซื้อสำเร็จ'),
    purchaseThanks: (name: string) => bilingual(`Thanks for supporting Refinery Story! (${name})`, `ขอบคุณที่สนับสนุน Refinery Story! (${name})`),
    products: {
      remove_ads: { title: bilingual('Remove Ads', 'ลบโฆษณา'), description: bilingual('Removes all ads, forever. One-time purchase.', 'ลบโฆษณาทั้งหมดถาวร ซื้อครั้งเดียว') },
      rewarded_cash: { title: bilingual('Watch Ad for Cash', 'ดูโฆษณารับเงิน'), description: bilingual('Watch a short video ad for an instant cash bonus.', 'ดูวิดีโอสั้น ๆ เพื่อรับโบนัสเงินทันที') },
      rewarded_double: { title: bilingual('Watch Ad for 2x Boost', 'ดูโฆษณารับบูสต์ 2 เท่า'), description: bilingual('Watch a short video ad to double contract/event rewards for a limited time.', 'ดูวิดีโอสั้น ๆ เพื่อเพิ่มรางวัลสัญญา/เหตุการณ์เป็น 2 เท่าชั่วคราว') },
      cash_small: { title: bilingual('Starter Cash Pack', 'แพ็กเงินเริ่มต้น'), description: bilingual('A small cash injection to speed up your early game.', 'เติมเงินก้อนเล็กเพื่อเร่งช่วงต้นเกม') },
      cash_large: { title: bilingual('Investor Cash Pack', 'แพ็กเงินนักลงทุน'), description: bilingual('A larger cash injection for mid-game expansion.', 'เติมเงินก้อนใหญ่สำหรับขยายช่วงกลางเกม') },
      rp_pack: { title: bilingual('Research Grant', 'ทุนวิจัย'), description: bilingual('Bonus research points to unlock upgrades faster.', 'แต้มวิจัยโบนัสเพื่อปลดล็อกอัปเกรดเร็วขึ้น') },
    },
  },
  onboarding: {
