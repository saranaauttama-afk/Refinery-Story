...
export function V3MidgamePanels({ state, apply, t, describe }: Props) {
  ...
  const labBuildingId = buildings.find((building) => building.type === 'laboratory')?.id ?? null
  const rank = labBuildingId ? getV3AvailableKnowledgeRank(state, labBuildingId) : 0
        ...
        {PORTED.map((family) => {
          ...
          return (
            <Gate
              key={`sell-${family}`}
              label={t({
                en: `Sell ${quantity} ${FAMILY_LABEL[family].en} · ~$${(quantity * V3_SPOT_PRICE_CENTS[family] * getV3MarketMultiplier(state, family) / 100).toFixed(0)}`,
                th: `ขาย ${FAMILY_LABEL[family].th} ${quantity} หน่วย · ~$${(quantity * V3_SPOT_PRICE_CENTS[family] * getV3MarketMultiplier(state, family) / 100).toFixed(0)}`,
              })}
        ...
        {state.developmentProject ? (
          ...
          <Gate
            ...
            action={{
              ...
              knowledgeRank: rank, leadEmployeeId: null, labBuildingId: labBuildingId ?? '',
            }}
          />
        ))}
      </View>
