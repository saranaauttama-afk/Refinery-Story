  ...
  return (
    <>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.row}>{mark(period.qualifiedUnits >= period.deliveryTarget)} {t({ en: `Deliver ${period.deliveryTarget} units to customer orders (spot sales do not count): ${period.qualifiedUnits.toFixed(0)}/${period.deliveryTarget}`, th: `ส่งสินค้าเข้างานลูกค้าให้ครบ ${period.deliveryTarget} หน่วย (ขายตลาดไม่นับ): ${period.qualifiedUnits.toFixed(0)}/${period.deliveryTarget}` })}</Text>
        <Text style={styles.row}>{mark(period.qualifiedFamilies.length >= period.varietyTarget)} {t({ en: `Deliver ${period.varietyTarget} different product type(s): ${period.qualifiedFamilies.length}/${period.varietyTarget}`, th: `ส่งสินค้าต่างชนิดกัน ${period.varietyTarget} ชนิด: ${period.qualifiedFamilies.length}/${period.varietyTarget}` })}</Text>
        <Text style={styles.row}>{mark(live.profitCents > 0)} {t({ en: `Make an operating profit this period: ${money(live.profitCents)}`, th: `มีกำไรดำเนินงานในรอบนี้: ${money(live.profitCents)}` })}</Text>
